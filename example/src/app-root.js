 ...
 *
 * An application usually puts its sidebar, header and breadcrumb here. This one does
 * not, because two of its routes must render without chrome. `/login`, which nobody
 * signed in reaches, and the standalone error pages. A shell that rendered the chrome
 * unconditionally would then have to hide it with a condition on every part of it.
 *
 * So the chrome is a route. `shell-layout.js` is a layout route whose children are
 * every screen inside the application, and it carries `requireSession` for all of them
 * at once. That is Angular's `AppComponent` holding only a `<router-outlet>`, with the
 * shell one level down, and it buys three things.
 *
 *   - One guard for the whole authenticated area, rather than one per leaf.
 *   - A login screen that is a full page rather than a page pretending the sidebar is
 *     not there.
