...
/**
 * The route table. Angular's `Routes`, with every kind of entry appearing once.
 *
 ...
 *                  `inventory` and `people`. It contributes a path prefix and nothing
 *                  else, which is what a section without a page of its own is.
 ...
 *   catch-all      last, as it must be, because first match wins.
 *
 ...
 * leaves that need more than a session and are an affordance rather than the boundary,
 * because the server enforces the same scope on every request. ADR-0065.
 *
 * @returns {RouteDef[]}
 */
export function createRoutes() {
  return [
    // Outside the shell, and the one route a signed-out visitor is allowed to
    // render.
      ...
      children: [
        ...
        /* ── Sales ────────────────────────────────────────────────────────── */
        {
          ...
          children: [
            ...
            {
              // A layout of its own, so the order's header stays mounted while the
              // three tabs replace each other and switching tabs neither refetches
             ...
             * `?edit=true` separates them, because a query change on a matched route
             ...
             *
             * That puts the entitlement in two places rather than one.
             * `customers/new` is still a guarded route, because a create has nothing
             * to show a reader. `customers/:id` is guarded for reading, and the edit
             * affordance inside it is what checks `sales:write`, because a query
             * parameter cannot be a route guard. The server refuses the write either
             * way, so the guard is usability and `example/server/api.mjs` is the
             * boundary.
             *
        ...
        /*
         * Where every permission denial lands, from `requireScope` above and from
         * the mount guards the manifest's `requires` blocks become. Not guarded
         * itself beyond the session, and it says nothing about what was missing,
         * because a page that enumerated the entitlement someone lacks is a page that
 ...
 *
 * Written here rather than as a method the router calls by name, because the question
 * belongs to the route. The same component behind a route that did not want the prompt
 * would not get one. The screen answers it, synchronously with `true` when there is
 * nothing unsaved and otherwise with a promise it resolves when the user has answered
 * the prompt it renders.
 *
 * The `import` is inside the guard on purpose. Naming the class at the top of this
 * module would fetch the form's code during startup and undo the `load` three lines
 * above it. By the time a level can be deactivated its module is already in the module
 * map, so this resolves from cache and costs a microtask. That buys a real `instanceof`,
 * which is what stops a renamed method from quietly turning the guard into "yes,
 * always".
 *
