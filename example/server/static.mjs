/**
 * The static half of this application's server, with the same three mounts the dev
 ...
 * adapter over it beside `cli/dev/serve.mjs`, the benchmark origin and the artifact
 * test origin (ADR-0075). A fifth implementation of the mount walk, the traversal
 * refusal, the history fallback and the content type would be the copy that mattered
 * most, because this is the server `npm run example:serve` starts and so the one a
 * developer of this application is looking at. ADR-0081.
 *
 * What this adapter states, and nothing else:
 *
 *   templates     `app.manifest.json` announced with `templateFiles`, computed
 *                 from `cli/project-model/`, the same key the build writes from the
 *                 artifact it emitted, read by the same startup step. Without
 ...
 *                 in development, because `--api-only` never imports this module,
 *                 and the session is what makes the two servers one behaviour rather
 *                 than two, which is the same argument the mount table already won.
 *
 * This application serves its own files rather than letting `npm start` do it because
 * the API, the auth cookie and the event stream must be same-origin with the page. `SameSite=Strict` on the session cookie, `grants.api` comparing a remote's
 ...
 * watches and open event streams, and a process that ends without releasing them waits
...
export function staticOrigin(directory, options = {}) {
  // `resolve` rather than the argument as given, because server.mjs builds it from a
  // URL so it arrives with a trailing separator, and the project model compares
  // directory prefixes to decide which URL a template file is served at. `<dir>/` plus
  // `/` is a prefix nothing matches, which silently announces only the mounted
  // library's templates and none of the application's own.
