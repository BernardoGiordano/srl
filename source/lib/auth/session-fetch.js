 ...
 *
 * `core/` may not import `auth/`, the same rule that keeps `core/remotes/mfe.js` free
 * of the session, so the HTTP client takes its transport as a parameter. This is the
 * adapter for the ordinary case, an application whose API calls are the signed-in
 * user's. It is one line, and it is in the library rather than in each `main.js`
 * because of what the line has to get right.
 *
