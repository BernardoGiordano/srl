...
the mounts, the specifier prefixes, the vendored dependencies. The `exports` map, the
generated import-map fragment, each application's inline map and the root `tsconfig` paths
are all derived from it or checked against it. Adding a layer is one edit there, not five.
