...
and p95" while the code compared medians only. The editor target is a p95, because 100 ms
is the figure a keystroke is judged against rather than a typical case, so the two had to be
reconciled before an editor number could be gated at all.
...
the ordering facts, that a completion is answered before the validation it queued and that
a withdrawn request gets `-32800`, and the timings are the harness's.
