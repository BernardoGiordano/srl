...
windowing is justified** — because no product budget exists to fail (below). Sticky
...
| `product` | empty | No absolute limits. A product limit is compared raw: no speed scaling, no noise slack |
...
`product` is empty on purpose, and that is a decision rather than a deferral. Absolute
limits set near this machine's medians would fail on any slower machine and on every busy
moment here — the busy population measured 1.46x to 2.63x — and a gate that reds for the
environment teaches people to ignore it. Absolute limits need a known target machine and a
known target application scale. Neither is fixed, so the relative gate carries the work,
and the consequence stays visible: with no required budget, the row-windowing question is
