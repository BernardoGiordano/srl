...
source/package.json         THE LIBRARY'S INTERFACE: the mounts a browser sees, the
                            specifier prefixes source is written against, the vendored
                            dependencies, and the `exports` map a registry consumer
                            resolves through. `source/` is the package; the repository
                            around it is one consumer of it
...
example/                    THE APPLICATION, served by its own Node backend
  ...
  app.css                   committed production stylesheet
