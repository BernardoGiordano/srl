 ...
 * code up without a run in front of them. `srl check --codes` prints the table.
 *
 * A test keeps the table and the checks in step, in both directions. A check that
 * reports an uncatalogued code fails it, and so does an entry no check reports.
 */

/** Codes whose tail is a TypeScript error number, such as `types/ts2339`. */
export const CODE_FAMILIES = Object.freeze({
  'types/ts': 'TypeScript reports this error in a file tsconfig.json includes. The number is TypeScript\'s own.',
  'templates/ts': 'TypeScript reports this error in a template binding, placed at the binding. The number is TypeScript\'s own.',
});
...
/**
 * The sentence for one code, or undefined when the code is not catalogued.
 *
 * @param {string} code
 * @returns {string | undefined}
 */
export function describeCode(code) {
  const exact = /** @type {Record<string, string>} */ (CODES)[code];
  if (exact !== undefined) return exact;
  for (const [prefix, summary] of Object.entries(CODE_FAMILIES)) {
    if (code.startsWith(prefix) && /^\d+$/u.test(code.slice(prefix.length))) return summary;
  }
  return undefined;
}
...
/**
 * The catalogue as one JSON document.
 *
 * @returns {string}
 */
export function formatCatalogJson() {
  return `${JSON.stringify({ codes: CODES, families: CODE_FAMILIES }, null, 2)}\n`;
}
