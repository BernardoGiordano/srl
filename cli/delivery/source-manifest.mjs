/**
 * What source delivery announces to the runtime, which is the checked-in
 * `app.manifest.json` plus the template list the runtime otherwise has no way to
 * learn while a developer works.
 *
 ...
 * two differ only in where the list is read from, an artifact the build just emitted
 * or `cli/project-model/` over the source on disk.
 *
 * `startApplication`'s third step warms the template cache from whichever key
 * `app.manifest.json` carries, and without this only a built artifact carries one.
 * `cli/delivery/build.mjs` writes `templateFiles` from what it emitted. A source
 * manifest with no such key skips the step, and every template is discovered the
 * slow way, where a chunk's module body runs, learns its own template URL, awaits
 * it, and only then does the next module body run. Nine components in a row is nine
 * round trips in a row, on every reload. ADR-0081.
 *
 * It is not a development-only key and not a second runtime path. The browser cannot
 ...
 * `prefetchTemplates` is the same function either way. One list and two producers is
 * also what makes a delivery bug show up while editing rather than at build.
 *
 * A Remote's markup is announced on that Remote's own entry and nowhere else, the
 * way the build announces it through
 * `templateAnnouncement(templateOutput, publicationBase)`. The shell must not
 * prefetch a Remote's templates, because the router runs the Remote's guard before
 * `prepareRemote` and a shell that had already fetched the markup would have spent
 * the request the guard exists to refuse. A template is attributed to the Remote
 * whose entry module it sits beside.
 *
 * It declines by returning null, which streams the file on disk unchanged, and never
 * by failing the request. Three cases reach that, and none should cost a developer
 * their server. The application configured `templateBundle` by hand, in which case
 * the runtime seeds from the bundle and a list beside it would be dead weight. The
 * project model threw, which mid-edit is an ordinary thing for a half-typed module
 * to do. Or `typescript` is not installed, which is why the model is imported lazily
 * and a server that depends on this still starts on a fresh clone.
 */
...
/**
 * The publication base of one Remote entry, such as `/remotes/billing/`, or null for
 * an entry that names no root-absolute URL.
 *
 * Read off `url` rather than `mount`. `mount` is the route the Remote owns and `url`
 * is where its bytes are, and only the second says anything about where its markup
 * sits. The build enforces that the two agree with `/remotes/<name>/`
 ...
 * the bytes can, which is the difference between the one generated document on the
 * page being a whole body on every reload and being a 304 like everything else.
 *
 * The model is rebuilt per request rather than cached against a watcher. It costs
 * 5 ms warm, because `cli/project-model/parse.mjs` caches each parse by path, size
 * and mtime, so a rebuild after one edit re-parses one file. A cache with no
...
export function templateAnnouncer(app, log) {
  ...
  const build = async () => {
    ...
    // The runtime prefers a bundle over a list, because seeding puts the markup in
    // the cache from bytes already in hand. A list beside one is requests for
